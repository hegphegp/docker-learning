...
vagrant box add CentOS-7-x86_64-Vagrant-1804_02 CentOS-7-x86_64-Vagrant-1804_02.VirtualBox.box

# 步骤02 编写Vagrantfile文件
rm -rf CentOS-7-x86_64-Vagrant-1804_02
mkdir -p CentOS-7-x86_64-Vagrant-1804_02
cd CentOS-7-x86_64-Vagrant-1804_02
...
Vagrant.configure("2") do |config|
  config.vm.define :centos1804 do |centos1804|
    centos1804.vm.box = "CentOS-7-x86_64-Vagrant-1804_02"
    centos1804.vm.hostname = "centos-1804"
# 官方镜像都不能设置账号密码登录，因为官方原始镜像的/etc/ssh/sshd_config文件配置都是不允许任何账号远程登录的
    centos1804.vm.synced_folder ".", "/vagrant", disabled: true
    centos1804.vm.network :private_network, ip: "192.168.130.11"
    centos1804.vm.provider "virtualbox" do |vb|
      vb.customize [ "modifyvm", :id, "--name", "CentOS-7-x86_64-Vagrant-1804_02", "--memory", "1024", "--cpus", "2" ]
    end
  end
end
EOF
...
# 步骤05 清空网络配置，然后作为模板
# 网上说，要删除 /etc/udev/rules.d/70-persistent-net.rules ，用官方的CentOS-7-x86_64-Vagrant-1804_02.VirtualBox.box创建虚拟机，/etc/udev/rules.d目录下面没有任何文件
# 用官方的CentOS-7-x86_64-Vagrant-1804_02.VirtualBox.box创建虚拟机后，/etc/sysconfig/network-scripts/ifcfg-eth0网卡没有写定什么参数，/etc/sysconfig/network-scripts/ifcfg-eth1网卡配置了IP，应该删掉
...
# 步骤07 删除virtualbox的centos-1804虚拟机
vagrant destroy -f centos1804
cd ..
rm -rf CentOS-7-x86_64-Vagrant-1804_02
