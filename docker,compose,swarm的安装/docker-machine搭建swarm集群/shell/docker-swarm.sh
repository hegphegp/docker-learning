...
# 定义两个数组，分别保存manager和worker节点的名称
# >>>>>>>>>>>>定义开始变量
...
echo -e "\033[31m\n 步骤四:从manager节点获取第一台docker虚拟机作为管理节点,然后用manager1初始化swarm集群 \n\033[0m"
...
echo -e "\033[31m\n 步骤五:获取添加manager的token，然后把所有manager加入到swarm集群 \n\033[0m"
...
echo -e "\033[31m\n 步骤六:获取添加worker的token，然后把所有manager加入到swarm集群 \n\033[0m"
...
# 用时间换来的代价：awk结果转成数组, 然后用for遍历与直接用for遍历自定义的数组的写法不一样
...
#   do
# #   时间换来的代价 1) if 与[ 之间必须有空格 2) [ ]与判断条件之间也必须有空格 3) ]与; 之间不能有空格, ';'分号不可以少
